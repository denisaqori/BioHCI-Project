...
class NNCrossValidator(CrossValidator):
	...
	def __init__(self, subject_dict, data_splitter, dataset_processor, parameter, learning_def, all_categories):
		...
		super(NNCrossValidator, self).__init__(subject_dict, data_splitter, dataset_processor, parameter, learning_def,
											   all_categories)
	...
	def _get_data_and_labels(self, subj_dataset):
		...
		print("Using the PyTorch DataLoader to load the training data (shuffled) with: \nbatch size = ",
			  self._learning_def.get_batch_size(), " & number of threads = ", self._parameter.num_threads)
	...
	# to the respective list in the CrossValidator object
	def train(self, train_dataset):
		train_data_loader = self._get_data_and_labels(train_dataset)
		trainer = Trainer(train_data_loader, self.all_int_categories, self._learning_def, self.parameters)
	...
	def val(self, val_dataset):
		...
		model_name = self._parameter.study_name + "-" + self._learning_def.get_model_name() + "-batch-" \
