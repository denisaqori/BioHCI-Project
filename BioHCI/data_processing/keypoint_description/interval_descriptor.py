...
class IntervalDescription:
    ...
    def _describe_each_point_2D(self, keypoint_neighborhood_2D, nb, a):
        ...
        filtered_gradient_cols = gradient_cols
        ...
        col_stats = self._get_col_stats(keypoint_neighborhood_2D)
        mean_col_stats = np.mean(col_stats, axis=0)
        descriptor = np.concatenate((all_gradient_sums, mean_col_stats))
        # return all_gradient_sums
        return descriptor

    def _get_col_stats(self, keypoint_neighborhood_2D):
        col_diff = np.diff(keypoint_neighborhood_2D, n=1, axis=1)
        col_sum = np.sum(keypoint_neighborhood_2D, axis=1).reshape(col_diff.shape)
        col_mean = np.mean(keypoint_neighborhood_2D, axis=1).reshape(col_diff.shape)
        col_std = np.std(keypoint_neighborhood_2D, axis=1).reshape(col_diff.shape)

        arr = np.concatenate((col_diff, col_sum, col_mean, col_std), axis=1)
        return arr
